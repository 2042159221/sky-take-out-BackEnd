...
class OrderServiceImplTest {
    ...
    @Test
    @DisplayName("测试店铺地址解析失败")
    void testCheckOutOfRange_ShopAddressResolveFailed() throws Exception {
        // 准备测试数据
        String testAddress = "广东省广州市黄埔区九龙街区凤福花园9栋";
        
        // Mock百度地图API响应 - 店铺地址解析失败
        String shopCoordinateResponse = createFailedGeocodeResponse();
        ...
        try (MockedStatic<HttpClientUtil> mockedHttpClient = mockStatic(HttpClientUtil.class)) {
            ...
            assertEquals("店铺地址解析失败", exception.getMessage());
        }
    }

    @Test
    @DisplayName("测试客户地址解析失败")
    void testCheckOutOfRange_UserAddressResolveFailed() throws Exception {
        // 准备测试数据
        String testAddress = "无效地址";
        
        // Mock百度地图API响应 - 店铺地址解析成功
        String shopCoordinateResponse = createSuccessfulGeocodeResponse("113.123456", "23.123456");
        
        // Mock百度地图API响应 - 客户地址解析失败
        String userCoordinateResponse = createFailedGeocodeResponse();
        ...
        try (MockedStatic<HttpClientUtil> mockedHttpClient = mockStatic(HttpClientUtil.class)) {
            ...
            assertEquals("收货地址解析失败", exception.getMessage());
        }
    }

    @Test
    @DisplayName("测试路线规划失败")
    void testCheckOutOfRange_RoutePlanningFailed() throws Exception {
        // 准备测试数据
        String testAddress = "广东省广州市黄埔区九龙街区凤福花园9栋";
        ...
        // Mock百度地图API响应 - 客户地址解析成功
        String userCoordinateResponse = createSuccessfulGeocodeResponse("113.124456", "23.124456");
        
        // Mock路线规划API响应 - 失败
        String routeResponse = createFailedRouteResponse();
        ...
        try (MockedStatic<HttpClientUtil> mockedHttpClient = mockStatic(HttpClientUtil.class)) {
            ...
            assertEquals("路线规划失败", exception.getMessage());
        }
    }

    @Test
    @DisplayName("测试提交订单时的地址检查集成")
    void testSubmitOrder_AddressCheck() {
        // 准备测试数据
        ...
        addressBook.setCityName("广东省广州市");
        addressBook.setDistrictName("黄埔区");
        addressBook.setDetail("九龙街区凤福花园8栋");
    ...
    @Test
    @DisplayName("测试地址为空的情况")
    void testSubmitOrder_AddressNotFound() {
        // 准备测试数据
        OrdersSubmitDTO ordersSubmitDTO = new OrdersSubmitDTO();
        ordersSubmitDTO.setAddressBookId(999L);
        ...
        assertEquals("地址簿数据不存在", exception.getMessage());
    }

    @Test
    @DisplayName("测试边界值 - 距离正好5000米")
    void testCheckOutOfRange_ExactlyAtBoundary() throws Exception {
        // 准备测试数据
        String testAddress = "广东省广州市黄埔区某地址";
        
        // Mock百度地图API响应
        String shopCoordinateResponse = createSuccessfulGeocodeResponse("113.123456", "23.123456");
        String userCoordinateResponse = createSuccessfulGeocodeResponse("113.124456", "23.124456");
        String routeResponse = createSuccessfulRouteResponse(5000); // 正好5000米
    ...
    @Test
    @DisplayName("测试边界值 - 距离5001米")
    void testCheckOutOfRange_JustOverBoundary() throws Exception {
        // 准备测试数据
        String testAddress = "广东省广州市黄埔区某地址";
        ...
        String shopCoordinateResponse = createSuccessfulGeocodeResponse("113.123456", "23.123456");
        String userCoordinateResponse = createSuccessfulGeocodeResponse("113.124456", "23.124456");
        String routeResponse = createSuccessfulRouteResponse(5001); // 超出1米
    ...
    // 辅助方法：创建失败的地理编码响应
    private String createFailedGeocodeResponse() {
        JSONObject response = new JSONObject();
    ...
    // 辅助方法：创建失败的路线规划响应
    private String createFailedRouteResponse() {
        JSONObject response = new JSONObject();
        response.put("status", "1");
        response.put("message", "路线规划失败");
        return response.toJSONString();
    }
}